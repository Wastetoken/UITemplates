...
function createParticles(w, h, count, clearR) {
...
function createGlowSprite(warm) {
...
export function Erosion({
  ...
  const generateFilmGrain = useCallback((ctx, w, h) => {
  ...
  useEffect(() => {
    ...
    const startDelay = clearRadius > 0 ? 0.1 : 0.5;

    els.forEach((el, i) => {
      const chars = el.querySelectorAll(".erosion-char");
      if (chars.length > 0) {
        gsap.fromTo(chars,
          { y: 100, opacity: 0, rotateX: 120, scale: 0.7 },
          {
            y: 0,
            opacity: 1,
            rotateX: 0,
  ...
  const handleClick = useCallback(
    (e) => {
      if (!shockwaveOnClick || disableErosionClick) return;
      const c = containerRef.current;
      if (!c) return;
      const rect = c.getBoundingClientRect();
      const dpr = window.devicePixelRatio || 1;
    ...
    [shockwaveOnClick, disableErosionClick],
  );
  ...
  return (
    <div
      ...
      onClick={handleClick}
      style={{ cursor: "crosshair", background: isDark ? "#050505" : "#f5f2ee" }}
    >
...
export function ErosionText({ children, className, as: Tag = "h1" }) {
  return (
    <Tag data-erosion-reveal className={`overflow-hidden ${className || ''}`} style={{ perspective: "800px" }}>
...
export function ErosionPreview({ onEnter }) {
  ...
  return (
    <div ref={wrapperRef} className="relative w-full h-screen" style={{ background: isDark ? "#050505" : "#f5f2ee" }}>
      <Erosion
        key={key}
        particleCount={5000}
        erosionRadius={140}
      ...
      >
        <div className="flex flex-col items-center gap-6 px-8 text-center">
          <ErosionText
            as="h1"
            className={`text-[clamp(3rem,10vw,8rem)] font-extralight tracking-[-0.06em] leading-[0.85] ${isDark ? "text-white" : "text-[#1a1a1a]"}`}
          >
            Not A Club
          </ErosionText>
          <ErosionText
            as="p"
            className={`text-[clamp(0.875rem,1.5vw,1.125rem)] font-light tracking-[0.02em] max-w-[480px] ${isDark ? "text-white/50" : "text-black/45"}`}
          >
            Good taste by default
          </ErosionText>
          <div data-erosion-reveal className="mt-6 flex items-center gap-3 cursor-pointer z-50" onClick={handleEnterClick}>
            <div className={`h-px w-10 ${isDark ? "bg-white/15" : "bg-black/10"}`} />
            <span
              className={`text-[11px] uppercase tracking-[0.2em] font-light ${isDark ? "text-white/25" : "text-black/30"}`}
            >
              Click to enter
            </span>
            <div className={`h-px w-10 ${isDark ? "bg-white/15" : "bg-black/10"}`} />
      ...
      <button
        ...
        className={`absolute bottom-4 right-4 z-40 px-3 py-1.5 text-[20px] uppercase tracking-[0.15em] rounded-md backdrop-blur-sm transition-colors ${
          isDark
